...
const ACTIVE_CERTIFIED_USERS_QUERY = defineQuery(`{
  ...
  "certifiedPrevious30": count(*[_type == "progress" && count(exams[pass == true]) > 0 && dateTime(_updatedAt) <= dateTime(now()) - $SECONDS_IN_30_DAYS && dateTime(_updatedAt) > dateTime(now()) - $SECONDS_IN_60_DAYS])
}`)
